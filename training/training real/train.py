...
from training.cross_validation import run_kfold
...
def main() -> None:
    ...
    config["k_fold"]["enabled"]가 True이면 K-Fold Cross Validation을 실행하고
    (STEP6.5), False이면 기존 Single Train/Validation을 그대로 실행한다.
    """
    args = parse_args()
    config = load_config(args.config)

    # STEP6.5: config.k_fold.enabled=True이면 K-Fold로 분기하고 여기서 종료
    if config.get("k_fold", {}).get("enabled", False) and not args.dry_run:
        logger.info("k_fold.enabled=True -> K-Fold Cross Validation으로 실행합니다.")
        run_kfold(config)
        return
